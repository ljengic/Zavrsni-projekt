...
Drugi parametar koji opisuje zadatak je trajanje njegova izvršavanja $C_i$ (\textit{engl. computation time}). Period i vrijeme izvršavanja zajedno daju veličinu 
koju nazivamo faktor opterećenja (\textit{engl. utilization factor}):
\begin{equation*}
...
Nadalje, bitna veličina koja opisuje svaki zadatak je krajnji rok njegova završetka (\textit{engl. deadline}). Krajnji rok izvršavanja je vrijeme 
...
U ovom radu ispitivat će se ponašanje sustava s ublaženo-strogim uvjetima (\textit{engl. weakly-hard constraints}). Za razliku od 
...
U ovom radu istražen je model u kojem se u $S_i$ slijednih perioda zadatak $\tau_i$ ne smije propustiti više od jednom (\textit{engl. skip-over model}).
Broj $S_i$ naziva se faktor propuštanja (\textit{engl. skip-over factor}).
Prema tom modelu 
...
\begin{itemize}
  \item[--] ako se plavi posao ne izvede na vrijeme, slijedećih $S_i-1$ poslova se proglašava crvenima,
...
Algoritam EDF (\textit{engl. earliest deadline first}) je algoritam koji pri raspoređivanju zadataka najveći prioritet daje onim poslovima 
...
Algoritam RTO (\textit{engl. red tasks only}) je prvi i najjednostavniji algoritam korišten za skip-over model SRSV-a s ublaženo-strogim uvjetima.
...
Algoritam BWP (\textit{engl. blue when possible}) je poboljšanje ranije opisanog RTO algoritma. Kod BWP algoritma prioritet imaju strogi zadatci, 
