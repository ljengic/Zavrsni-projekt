...
Zadatci u FreeRTOS-u su opisani strukturom za upravljanje zadatcima (\textit{engl. TCB - task control block}). U toj strukturi nalaze
...
procesorskog vremena među zadatcima (\textit{engl. time slicing}).
...
Kao što je ranije rečeno, u ovom radu je razmatran skip-over model SRSV-a s ublaženo-strogim uvjetima.
...
Vrijednost varijable \texttt{xItemValue} mijenja se pozivom macro funkcije \texttt{listSET\_LIST\_ITEM\_VALUE()} 
...
\begin{lstlisting}[style=CStyle,caption={Pseudo kod implementacije algoritma EDF},captionpos=b]
PESUDO KODDD
\end{lstlisting}
...
\begin{lstlisting}[style=CStyle,caption={Pseudo kod implementacije algoritma RTO},captionpos=b]
  PESUDO KODDD
\end{lstlisting}
...
\begin{lstlisting}[style=CStyle,caption={Pseudo kod implementacije algoritma BWP},captionpos=b]
  PESUDO KODDD
\end{lstlisting}
